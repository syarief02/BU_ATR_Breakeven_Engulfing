...
int CountBuy() {
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--) { // Loop through all orders from the last to the first
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { // Select the order by position
            if (OrderType() == OP_BUY && OrderSymbol() == Symbol()) { // Check if the order is a buy order for the current symbol
...
int CountSell() {
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--) { // Loop through all orders from the last to the first
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { // Select the order by position
            if (OrderType() == OP_SELL && OrderSymbol() == Symbol()) { // Check if the order is a sell order for the current symbol
...
{
    if (!newbar()) return; // Check for a new bar; if not, exit the function
    ...
    if (isBullishEngulfing(1) && rsiValue >= RSIBuy && CountBuy() < MaxLayer) { // If a bullish engulfing pattern is detected and RSI is above the buy threshold
        OpenBuyOrder(); // Attempt to open a buy order
    }
    
    // Check for bearish engulfing pattern with RSI filter
    if (isBearishEngulfing(1) && rsiValue <= RSISell && CountSell() < MaxLayer) { // If a bearish engulfing pattern is detected and RSI is below the sell threshold
...
bool newbar() {
    ...
    if (currentTime != lastTime) { // Check if the current time is different from the last recorded time
...
void OpenBuyOrder() {
    ...
    if (ticket < 0) { // Check if the order was not successful
...
void ApplyATRTrailingStop(int atrPeriod, double atrMultiplier) {
    ...
    for(int i=OrdersTotal()-1; i>=0; i--) { // Loop through all open orders from the last to the first
        if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { // Select the order by position
            if(OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol()) { // Check if the order belongs to this EA and is for the current symbol
                double newStop; // Variable to hold the new stop loss price
                if(OrderType() == OP_BUY) { // Check if the order is a buy order
                    newStop = NormalizeDouble(Bid - trailingPips * Point, Digits); // Calculate new stop loss price for buy order
                    if(newStop > OrderStopLoss()) { // Check if the new stop loss is higher than the current stop loss
                       ...
                       if(result) { // Check if the modification was successful
                ...
                } else if(OrderType() == OP_SELL) { // Check if the order is a sell order
                    newStop = NormalizeDouble(Ask + trailingPips * Point, Digits); // Calculate new stop loss price for sell order
                    if(newStop < OrderStopLoss()) { // Check if the new stop loss is lower than the current stop loss
                         bool result = OrderModify(OrderTicket(), OrderOpenPrice(), newStop, OrderTakeProfit(), 0, clrNONE); // Modify the order with the new stop loss
                        if(result) { // Check if the modification was successful
                            Print("Trailing stop applied successfully"); // Print success message
                        } else {
        ...
        if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { // Select the order by position
            if(OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol()) { // Check if the order belongs to this EA and is for the current symbol
                ...
                if(OrderType() == OP_BUY) { // Check if the order is a buy order
                    ...
                    if(distance >= breakevenPips && OrderStopLoss() < entryPrice) { // Check if the distance is greater than or equal to breakeven pips and current stop loss is below entry price
                    ...
                    if(distance >= breakevenPips && OrderStopLoss() > entryPrice) { // Check if the distance is greater than or equal to breakeven pips and current stop loss is above entry price
                         bool result = OrderModify(OrderTicket(), entryPrice, entryPrice, OrderTakeProfit(), 0, clrNONE); // Modify the order to set stop loss to breakeven
                        if(result) { // Check if the modification was successful
                            Print("Breakeven applied successfully"); // Print success message
                        } else {
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--) { // Loop through all orders from the last to the first
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { // Select the order by position
            ...
            if (OrderType() == OP_BUY) { // Check if the order is a buy order
                ...
                if (price > ma_value) { // If the current price is above the moving average
                    ...
                    if (Ask - new_stop_loss_buy < minStopLevel) { // Check if the new stop loss is too close to the ask price
                        new_stop_loss_buy = Ask - minStopLevel;  // Fallback to minimum stop level
                    }

                    // Modify SL if required
                    if (OrderStopLoss() < new_stop_loss_buy || OrderStopLoss() == 0) { // Check if the current stop loss is less than the new stop loss or if it is not set
                        if (new_stop_loss_buy > OrderOpenPrice()) { // Ensure the new stop loss is below the open price for buy orders
                            if (OrderModify(OrderTicket(), OrderOpenPrice(), new_stop_loss_buy, OrderTakeProfit(), 0, clrRed)) { // Modify the order with the new stop loss
                ...
                if (price < ma_value) { // If the current price is below the moving average
                    ...
                    if (new_stop_loss_sell - Bid < minStopLevel) { // Check if the new stop loss is too close to the bid price
                        new_stop_loss_sell = Bid + minStopLevel;  // Fallback to minimum stop level
                    }

                    // Modify SL if required
                    if (OrderStopLoss() > new_stop_loss_sell || OrderStopLoss() == 0) { // Check if the current stop loss is greater than the new stop loss or if it is not set
                        if (new_stop_loss_sell < OrderOpenPrice()) { // Ensure the new stop loss is above the open price for sell orders
                            if (OrderModify(OrderTicket(), OrderOpenPrice(), new_stop_loss_sell, OrderTakeProfit(), 0, clrRed)) { // Modify the order with the new stop loss
